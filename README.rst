...
* builds a PyWavelets wheel, linking against these fresh builds;
...
* making a commit to the `pywavelets-wheels` repository (e.g. with `git
...
``PyWavelets`` is the root name of the wheel(s) to download / upload, and
...
``PyWavelets-0.4.0-`` from http://wheels.scipy.org to ``~/wheelhouse``,
